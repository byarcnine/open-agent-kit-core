...
export const getAllPermissionsWithInheritance = (scopes: string[]) => {
  ...
  const processScope = (scope: string) => {
    if (visited.has(scope)) return; // Avoid circular dependencies
    visited.add(scope);
    ...
    if (scope.endsWith("*")) {
      ...
      matchingPermissions.forEach((perm) => {
        ...
        processScope(perm); // Recursive call for each matching permission
      });
    } else {
      const permissionConfig =
        AVAILABLE_PERMISSIONS[scope as keyof typeof AVAILABLE_PERMISSIONS];
      if (permissionConfig?.grants) {
        permissionConfig.grants.forEach((grant) => {
          allPermissions.add(grant);
          processScope(grant); // Recursive call for each grant
        });
      }
    }
  };

  // Process all initial scopes
