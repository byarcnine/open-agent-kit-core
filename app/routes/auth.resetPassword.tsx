...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  return { mode: "request", appURL };
};

export default function Screen() {
  const actionData = useActionData<{ error?: string; success?: string }>();
  const { token, email, mode, appURL } = useLoaderData<typeof loader>();
    ...
    if (mode === "reset") {
      ...
      try {
        await authClient.forgetPassword({
          email,
          redirectTo: appURL + `/auth/resetPassword?email=${email}`,
        });
        setSuccessMessage(
        ...
        setSuccessMessage(
          "There was an error sending the password reset link. Please try again or contact support.",
        );
      }
    }
  };
